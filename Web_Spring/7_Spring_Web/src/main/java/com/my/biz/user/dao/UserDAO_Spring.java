...
public class UserDAO_Spring implements UserDAO{
	...
	public UserDAO_Spring() {
		//spring = new JdbcTemplate();
			//JdbcTemplate은 DataSource필요

		/*DriverManagerDataSource dataSource = new DriverManagerDataSource();
			//dataSource->내부적으로 connection 관리
	...
	public int addUser(UserVO user) {
		...
		return spring.update(sql, user.getId(), user.getName(), user.getPw(), user.getRole());
	}
	...
	public int updateUser(UserVO user) {
		...
		return spring.update(sql, user.getName(), user.getPw(), user.getRole(), user.getId());
	}
